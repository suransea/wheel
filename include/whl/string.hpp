...
template<typename Str, typename C=std::vector<Str>, typename Regex>
constexpr inline C split(const Str &str, const Regex &regex) {
  std::basic_regex basic_regex(regex);
  return C{std::regex_token_iterator(std::begin(str), std::end(str), basic_regex, -1),
           std::regex_token_iterator<decltype(std::begin(str))>()};
}

template<typename C, typename Str=typename C::value_type, typename Dlm>
constexpr inline Str join(const C &coll, const Dlm &delimiter) {
  Str result = *std::begin(coll);
  return std::accumulate(++std::begin(coll), std::end(coll), result, [&delimiter](auto &&x, auto &&y) {
...
template<typename Str>
constexpr inline Str toupper(const Str &str) {
  Str result(str);
...
template<typename Str>
constexpr inline Str tolower(const Str &str) {
  Str result(str);
